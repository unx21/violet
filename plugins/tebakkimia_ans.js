...
handler.before = async function (m) {
    let id = m.chat
    if (!m.quoted || !m.quoted.fromMe || !m.quoted.isBaileys || !/Ketik.*tmhint/i.test(m.quoted.text)) return !0
    ...
    if (m.quoted.id == conn.tebakkimia[id][0].id) {
        ...
        if (m.text.toLowerCase() == json.name.toLowerCase().trim()) {
            global.DATABASE.data.users[m.sender].uang += conn.tebakkimia[id][2]
            m.reply(`*Benar!*\n+${conn.tebakkimia[id][2]}`)
